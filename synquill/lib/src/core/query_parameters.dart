...
class FilterCondition<T> {
  ...
  @override
  String toString() => 'FilterCondition(field: $field, operator: $operator, '
      'value: $value)';
}
...
class QueryParams {
  ...
  const QueryParams.filters(this.filters)
      : sorts = const [],
        pagination = null;

  /// Creates typed query parameters with only sorting.
  const QueryParams.sorts(this.sorts)
      : filters = const [],
        pagination = null;
  ...
  QueryParams mergeWithRequiredFilter(FilterCondition requiredFilter) {
    final filteredConditions = filters
        .where(
          (filter) => filter.field.fieldName != requiredFilter.field.fieldName,
        )
        .toList();
