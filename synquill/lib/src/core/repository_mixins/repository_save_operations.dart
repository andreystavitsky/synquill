...
mixin RepositorySaveOperations<T extends SynquillDataModel<T>>
    ...
    switch (savePolicy) {
      ...
      case DataSavePolicy.remoteFirst:
        ...
        if (localOnly) {
          ...
          emitSavedChange(item, isExisting: isExisting);
  ...
  Future<T> _handleStandardSave(
    ...
    switch (savePolicy) {
      case DataSavePolicy.localFirst:
        ...
        if (!localOnly) {
          await processBackgroundSync(item, operation, headers, extra);
        }
      ...
      case DataSavePolicy.remoteFirst:
        ...
        final operation =
            isExisting ? SyncOperation.update : SyncOperation.create;
        resultItem = await runRemoteFirstWriteTask<T>(
          execute: () => _saveToRemoteAndLocal(
            item,
          ...
          operation: operation,
          modelId: item.id,
          idempotencyKey: '${item.id}-remoteFirst-${operation.name}-${cuid()}',
          taskName: 'RemoteFirstSave-${T.toString()}-${item.id}',
          failureDescription: 'save $T ${item.id}',
        );
        break;
    }
    return resultItem;
  ...
  Future<T> _saveToRemoteAndLocal(
    T item, {
    ...
    // Ensure the remoteItem also has the repository set, especially if it is a
    // new instance from the adapter.
    remoteItem.$setRepository(this as SynquillRepositoryBase<T>);
    await saveToLocal(remoteItem);
    emitUpdatedChange(remoteItem);
  ...
  Future<T> _handleLocalFirstWithIdNegotiation(
    ...
    await saveToLocal(item);
    emitSavedChange(item, isExisting: false);
    ...
    startBackgroundTaskAndCleanup(syncTask, syncQueueId);
